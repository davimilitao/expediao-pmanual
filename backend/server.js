...
app.get('/bling/pedidos', async (req, res, next) => {
  try {
    ...
    const data = await blingGet(`/pedidos/vendas?${params}`);
...
app.get('/bling/pedidos/:id', async (req, res, next) => {
  try {
    const data = await blingGet(`/pedidos/vendas/${req.params.id}`);
...
// ── HELPER: detectar marketplace pelo campo loja/origem ───────────
function detectarMarketplace(p) {
  const loja  = (p.loja?.nome || p.canal?.nome || p.tipoIntegracao || '').toLowerCase();
  const num   = String(p.numeroPedido || p.numero || '');
  if (loja.includes('mercado') || loja.includes('ml') || num.length > 10) return 'MERCADO_LIVRE';
