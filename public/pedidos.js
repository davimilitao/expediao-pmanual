...
async function onScan(code) {
  ...
  sc.className = 'scanner-status busy';
  ...
  try {
    ...
    if (r && r.ok) {
      ...
      sc.className = 'scanner-status ready'; sc.textContent = '● PRONTO';
      await refreshAll();
    } else if (r) {
      beep(false); flash(false);
      const msgs = {
        item_not_found:           'Código não encontrado neste pedido',
        already_fully_checked:    'Item já foi totalmente conferido',
        locked_by_other_terminal: 'Pedido travado por outro terminal',
      };
